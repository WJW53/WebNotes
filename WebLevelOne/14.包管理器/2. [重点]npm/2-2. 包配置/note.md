...
1. 拷贝工程后如何还原？(node_module不在等问题)
2. 如何区分开发依赖 和 生产依赖？ (开发环境>=生产环境的依赖数目)
...
- devDependencies：仅开发环境的依赖包(在开发环境但不在生产环境里的)
...
> 自动保存的依赖版本，例如```^15.1.3```，这种书写方式叫做 语义版本号（semver version），具体规则后续讲解

`凡是自己开发阶段(服务器上不会用到的东西)的代码基本都是属于开发依赖`