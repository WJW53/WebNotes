...
每个chunk(意味着可能有多个chunk)都有至少两个属性：

- name：默认为main
- id：唯一编号，开发环境下和name相同，生产环境是一个数字，从0开始

2. **构建所有依赖模块,这块最复杂**

![](assets/2020-01-09-12-32-38.png)

`这里是递归加载dependencies(看成个数组)的内容中的模块,而不是运行`
...
接下来，webpack会根据配置为chunk生成一个资源列表，即 chunk assets，`资源列表可以理解为是生成到最终文件的文件名和文件内容(头疼在于可能不止一个文件)`
...
5. chunkhash：某一个chunk生成的资源清单内容联合生成的hash值
