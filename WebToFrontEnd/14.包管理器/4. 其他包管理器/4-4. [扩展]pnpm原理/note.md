...
- windows系统中即便目录很深,pnpm会让它变得可控的(只是看上去深),无论依赖关系多么深,其实是一样的深度的,而且现在pnpm已经解决了使用间接性依赖的问题
...
硬链接的概念来自于 Unix 操作系统，它是指将一个`文件A指针`复制到另一个文件B指针中，文件B就是文件A的硬链接
...
windows Vista操作系统开始，支持了创建硬链接的操作，在cmd管理员状态中(不能在PowerShell中)使用下面的命令可以创建硬链接

```shell
mklink /h 链接名称(这是要新创建的文件) 目标文件
```

`硬/符号链接命令里可以填写相对(最好填写相对路径)/绝对路径`
...
8. 完成(它的目录一定是可控的,最多就到最后那个node_modules了而已)