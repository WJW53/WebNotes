...
- 除了undefined、null、false、0、""这5个值以外,其他的都是真值

> 切换多个元素
- 因为 v-if 是一个指令，所以必须将它添加到一个元素上，但是如果想切换多个元素呢？此时可以把一个 ```<template>``` 元素当做不可见的包裹元素，并在上面使用 v-if。最终的渲染结果将不包含 ```<template>``` 元素(`就是不会影响HTML结构`) 
  ```html
  <template v-if="ok">
    <h1>Title</h1>
    <p>Paragraph 1</p>
...
`**所以，当我们处理复杂逻辑(例:多个值得到一个值)时，都应该使用计算属性。**`
...
### getter 读取
**在前面，我们直接将计算属性写成了一个函数，这个函数即为getter函数。也就是说，此时计算属性默认只有getter。**
...
```
`要注意，即使给计算属性赋了值，计算属性也不会改变，再重复一遍：只有当依赖的响应式属性变化了，计算属性才会重新计算。`
...
vm.obj.name = '老邓头'; // 此时回调函数会执行，控制台中打印出 ` 对象的值改变啦 `
```
> 上面这样节省效率,牛啊,还能这么写 'personObj.name' () {//代码}
...
```js
var unwatch = vm.$watch('msg', function () {//这里必须用var(let const都会报错的)
    // 干了点儿事
    if(unwatch) {
      console.log('xxx');
      unwatch();  
      return;//这里return时,即便第一次改变msg的值时也不会再打印haha了
    }
    console.log('haha');//这个haha无论如何都会打印一次的,因为watch初始化时就会打印它
    //这会儿unwatch还只是个undefined
  },{
    immediate: true
  }
})
...
3. `在侦听器中适合执行异步，但是在计算属性(它必须return完才异步)中不适合使用异步，例：`
...
```
通过上面方法，我们可以实现组件间的切换，能够注意到的是：每一次切换标签时，都会创建一个新的组件实例，重新创建动态组件在更多情况下是非常有用的，但是在这个案例中，我们会更希望哪些标签的组件实例能够在它们`第一次被创建的时候缓存下来`。为了解决这个问题，**我们可以用一个``<keep-alive>``元素将动态组件包裹起来。如：**
...
`provide 选项允许我们指定 想要提供给后代组件的数据/方法，`例如:
...
`然后在任何后代组件中，我们都可以使用 inject 选项来接受指定想要添加在实例上的属性。`
...
**注意：\$refs 只会在组件渲染完成之后生效，并且它们不是响应式的。应该避免在模板或计算属性中访问 \$refs。所以可在mounted钩子函数中访问**
...
会将组件的组织方式，耦合在一起，从而使组件重构困难，难以维护。不推荐在中大型项目中使用，适用于一些小组件的编写。

## eventBus(事件总线)
- 是我们自己写的组件的通信方法, 针对兄弟组件间的操作
```js
...
Vue.component('cmp-a', {
  ...
  template: `
    <div>
      <button @click="onClick">cmp-a</button>
    </div>
  `,
})
```
...
我们可以自己写一个自定义指令去操作DOM元素，以达到代码复用的目的。注意，在 Vue 中，代码复用和抽象的主要形式是组件。然而，`有的情况下，你仍然需要对普通 DOM 元素进行底层操作，这时候就会用到自定义指令v-xxx`。
...
- virtual node(VNode):虚拟的节点,是用一个对象模拟DOM元素,

### bind
**只调用一次，指令第一次绑定到元素时调用。在这里可以`进行一次性的初始化`设置。**

### inserted
被绑定元素插入父节点时调用(仅保证父节点存在，但不一定已被插入文档中)。

### update
**指令所在组件的 VNode 更新时调用，但是可能发生在其子 VNode 更新之前**。

### componentUpdated
**指令所在组件的 VNode 及其子 VNode 全部更新后调用。**

### unbind
只调用一次，指令与元素解绑时调用(调用时机：被绑定的DOM元素被Vue移除--比如v-if移除时)。


## 钩子函数参数

- `el: 指令所绑定的元素(模板中的样子)，可以用来直接操作DOM。`
- binding：对象，包含以下属性：
  - name：指令名，不包括 v- 前缀。
  ...
  -rawName：指令语句中等号前面的所有字符串，例如："v-focus:src.x.y"
  -def：我们写的所有的钩子函数
- `vnode：Vue 编译生成的虚拟节点，只需要记住其中的 context(是元素所在模板的vue实例) 属性`
...
Vue.directive('slice', {
  ...
  //因为slicenum是动态的，而bind只是一次性的，所以这个update中也要增加一个事件的监听
    el.oninput = function (e) {
      let inputVal = el.value;
...
## 函数简写
`当想在 bind 和 update 中触发相同行为，而不关心其他钩子时，可以把对象换成函数的形式`：
...
`过滤器可用在两个地方：双花括号插值 和 v-bind 表达式，添加在JS表达式的尾部，由“管道”符号 | 表示:`
...
Vue.filter('toMoney', value => {
  if(!value) { return };
  return value.toLocaleString();//toLocaleString()方法自动为数字每3位分配一个逗号
});
```
...
Vue.filter('addWord', value => {
  ...
  if(value >= 10000) {
    return ( value / 10000).toFixed(1) + '万';//toFixed(1)是只取小数点后1位数字
  }
  return value;
});
```
...
- 多元素过渡：就是切换展示每个需要过渡的元素,还是一个一个来

**`当切换展示的元素标签名相同时，需要给每一个元素设置不同的key值`，否则Vue为了效率只会替换相同标签内部的内容，而没有过渡效果。**
...
## 过渡模式
Vue提供一个 mode 特性(在transition上)，可以给多个元素过渡应用不同的模式，mode 的值可为：

- in-out：让新元素先进行过渡，完成之后当前元素过渡离开。某些情景下使用
- out-in：让当前元素先进行过渡，完成之后新元素过渡进入。平常用这个, 这个好看一些
...
## 列表的排序过渡
<transition-group> 组件提供了一个`新的特性：v-move，`它会在元素改变定位的过程中应用。
如何使用？通过类名即可设置：.v-move {}。
当给 <transition-group> 设置 name 特性时，例如name为fade，则 v-move 在使用时，需要替换为 fade-move。

注意：`当移除一个列表元素时，需要将移除的元素脱离文档流`，否则，要移除的元素在移除过渡中一直处于文档流中，会影响到后面元素的move过渡效果。

内部的实现：Vue 使用了一个叫 FLIP 简单的动画队列，使用 transforms 将元素从之前的位置平滑过渡新的位置。

**需要注意的是使用 FLIP 过渡的元素`不能设置为 display: inline` 。作为替代方案，可以设置为 display: inline-block 或者放置于 flex 中。**

## 列表的交错过渡
如果要给列表中的元素，应用更丰富的过渡效果，可以配合JavaScript钩子。还是velocity.js
...
# 过渡_复用过渡
**过渡可以通过 Vue 的组件系统实现复用。要创建一个可复用过渡组件，你需要做的就是将 <transition> 或者 <transition-group> 作为根组件，然后将任何子组件放置在其中就可以了（配合插槽）。**

注意：`当使用函数式组件复用过渡时，不要设置css作用域----style不要设置scoped，可以给transition的name赋值即可达到类似作用域的效果了。`
...
**上面代码注释那里的webpackChunkName就是要那个形式,不要把注释干掉！！！**

**`异步加载的文件，会在link标签上设置 rel="prefetch"。浏览器会在 空闲时间内 下载对应的资源，使用时可以 直接从缓存中 获取。与之对应的 rel="preload"，会及时/立刻 下载对应的资源。`**
...
<div id="app">
  ...
  <p>
    ...
    <!-- 通过传入 `to` 属性指定链接. -->
    <!-- <router-link> 默认会被渲染成一个 `<a>` 标签,设置tag可以改变为对应标签 -->
...
> 在VueRouter的配置对象中可以更改router-link 的 class 名
```js
VueRouter({
...
0. 页面级的组件要把相应文件都放入一个新的文件夹 view 中,router.js放到main.js同目录下即可
1. 不要放到main.js,新建一个router.js,然后在这里面配置路由,在这里面导出router然后在main.js中引入即可,还有render里
2. 很多页面都是应用异步组件,故在routes配置里的component应用异步组件即可
...
```js
{
  ...
  children: [
    {
      path: '/activity/academic',//可以简写,去掉与父路径重合的部分,最后那个/必须干掉,即删掉'/activity/'
      name: 'academic',
...
当访问 /activity 下的其他路径时，并不会渲染出来任何东西，如果想要渲染点什么，可以提供一个空路由(即默认渲染其中一个子路由的内容)：
```js
{
  path: '/activity',
  children: [
...
**其实这样效果欠佳,对于home来讲,还是可以用/和/home重定向,更好的解决**
...
### $router.replace
跟 router.push 很像，唯一的不同就是，`它不会向 history 添加新记录，而是替换掉当前的这条 history 记录`
...
当 URL 为 /foo/bar，\$router.matched 将会是一个包含从上到下的所有对象 (副本)。
...
```
## 路由的小缺陷
从这个路由页面底部点击回首页，回到首页后还是在底部，但实际上我们想要的是回到首页的顶部。因为我们实际上还是一个HTML，只是在模拟多页面切换，所以当首页有足够的空间，那么滚动条就不会滑到上面去。就会仍在底部呆着。
...
scrollBehavior 方法接收 to 和 from 路由对象。`第三个参数 savedPosition 当且仅当 popstate 导航 (通过浏览器的 前进/后退 按钮触发) 时才可用。浏览器支持这个那就可以用这个,浏览器不自带的话就用我们自己写的return`
...
Vue.use(Vuex);
//因为Vuex本身就是一个对象,所以得new它里面的Store构造函数
...
## State
单一状态树(仓库货架)，使用一个对象就包含了全部的应用层级状态。

> 1、`$store 是挂载在 Vue 实例上的（即Vue.prototype）`，而组件也其实是一个Vue实例（所有组件都是实例，每个组件都是一个vue实例，所有的 Vue 组件都是 Vue 实例：一个 Vue 应用由一个通过 new Vue 创建的根 Vue 实例，以及可选的嵌套的、可复用的组件树组成，也就是说：`组件放到根组件中使用`）`在组件中可使用 this 访问原型上的属性，template 拥有组件实例的上下文`，可直接通过 {{ $store.state.userName }̲} 访问，等价于 this.$store.state.userName。

> 2、所以通过store是挂载到vue上，为vue的根实例；store引入后被注入到vue上，成为vue的原型属性，所以通过store.state和this.$store.state可以访问。

### 在Vue组件中获得Vuex状态
**Vuex 通过store 选项，提供了一种机制将状态从根组件“注入”到每一个子组件中（调用Vue.use(Vuex)）。**
...
- mapState里面可以传一个数组/对象,结果返回一个对象,对象内的每个键值对就是一个计算属性(键值对为 <--> 属性名:函数)
...
```html
<template>
  <div>{{ $store.getters.addCount(3) }}</div>
</template>
```
```js
...
`之前能更改时因为我们是在vuex的非严格模式下，在严格模式下，只能是提交mutation`

**`不要在生产环境下开启严格模式!!!在开发环境下开启就行`**
...
不能直接调用一个mutation handler。这个选项更像是事件注册：“当触发一个类型为``increment``的mutation时，commit提交，调用此函数。”：
```js
...
- 为什么放在methods中,而不是computed中呢?
  - 因为我们想要的是函数,而且需要这些函数执行的,你放在computed中的是数据!
...
`大多数情况下，载荷应是一个对象，这样可以包含多个字段并且记录的mutation会更易读：`
```js
mutations: {
...
store.commit({
  type: 'increment',
  amount: 10,//所以这里不是payload,而是直接写参数即可
})
```
...
1. `对于模块内部的 mutation 和 getter`，接收的`第一个参数是模块的局部状态对象(填写对应state的名字就行)`。
